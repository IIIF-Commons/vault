...
for (const exportKey of exportKeys) {
  ...
  for (const variation of variations) {
    if (typeof pkg.exports[exportKey][variation] === 'string') {
      await checkFile(pkg.exports[exportKey][variation], variation);
    } else {
      logWithIndent(`"${variation}": {`);
      const specifics = Object.keys(pkg.exports[exportKey][variation]);
      indent();
      for (const specific of specifics) {
        await checkFile(pkg.exports[exportKey][variation][specific], specific);
      }
      dedent();
    }
  }
